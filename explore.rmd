...
midiNoteTable = read_csv("midiNoteTable.csv")
scaleDegrees = read_csv("scaleDegrees.csv")
enharmonicNotes = read_csv("enharmonicNotes.csv")
```
...
```{r}
saveMusic = function(wav_name, notes_pitch = NULL, notes_duration = NULL, tempo = 240) 
{
  . <- NULL
  note <- NULL
  octave <- NULL
  ...
  make_sine <- function(freq, duration) {
    ...
    n = max(2/(length(wave)-2)*sample_rate, 50)
  ...
  tune_wave <- mapply(make_sine, tune$freq, tune$duration, 
    SIMPLIFY = FALSE) %>% do.call("c", .)
  
  save.wave(tune_wave, wav_name)
}
```
...
soloMelodyNotes = soloMelodyNotes[1:100, ]
bassNotes = bassNotes[1:100, ]

saveMusic("melody.wav", notes_pitch=soloMelodyNotes$Note, tempo=bpm, notes_duration=soloMelodyNotes$duration*bpm/60)
saveMusic("bass.wav", notes_pitch=bassNotes$Note, tempo=bpm, notes_duration=bassNotes$duration*bpm/60)

melody = load.wave("melody.wav")
bass = load.wave("bass.wav")
