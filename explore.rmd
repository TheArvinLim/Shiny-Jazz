...
```{r}
getWave = function(notes_pitch = NULL, notes_duration = NULL, tempo = 240, note_shortness=100) 
{
  . <- NULL
  note <- NULL
  octave <- NULL
...
melody = getWave(notes_pitch=soloMelodyNotes$Note, tempo=bpm, notes_duration=soloMelodyNotes$duration*bpm/60, 75)
bass = getWave(notes_pitch=bassNotes$Note, tempo=bpm, notes_duration=bassNotes$duration*bpm/60, 500)

addToBass = rep(0, max(0, length(melody)-length(bass)))
addToMelody = rep(0, max(0, length(bass)-length(melody)))

combined = c(melody, addToMelody) + c(bass, addToBass)
